...
# Speed of light for IF-combination frequency handling (matches
# scripts/solve_ppp.py's C).  Kept local so antex is importable in
# isolation (for tests, for the regression runner pre-engine).
_C_LIGHT = 299_792_458.0


# Internal signal-name → ANTEX frequency-ID map.  Shared between
# regression harness and engine; covers signals emitted by the F9T
# driver (realtime_ppp.py) plus RINEX-standard codes.
_SIG_TO_ANTEX = {
...
def compute_pcv_correction(obs, sat_pos_ecef, receiver_ecef,
                           antex_parser, receiver_ant_type, epoch_t,
                           sun_pos_ecef=None):
    """IF-combined PCV+PCO range correction for one observation.

    Returns (Δrange_m, ok).  Add Δrange to pr_if AND phi_if_m (same
    signal path, same phase-center geometry) to correct the obs
    from ARP-to-ARP range to MPC-to-MPC range.  ``ok=False`` when
    any antenna pattern lookup misses (unknown signal, ANTEX lacks
    receiver or satellite entry); caller should skip applying.

    Args:
        obs: observation dict with at least ``sv``, ``f1_sig_name``,
            ``f2_sig_name``, ``wl_f1``, ``wl_f2``.
        sat_pos_ecef: satellite ECEF position, shape (3,).
        receiver_ecef: receiver ECEF position, shape (3,).
        antex_parser: ``ANTEXParser`` instance with loaded patterns.
        receiver_ant_type: 20-char ANTEX antenna type string, e.g.
            ``'TRM57971.00     NONE'``.
        epoch_t: UTC datetime for satellite-pattern validity lookup.
        sun_pos_ecef: optional Sun ECEF position (same frame as
            sat_pos_ecef) for full 3-axis body-frame projection of
            satellite PCO.  If None, falls back to U-only × cos(nadir)
            projection (adequate when body-Z dominates; typical PCO
            has Z ≈ 1-2 m vs body-X/Y at dm scale).
    """
    sv = obs['sv']
    f1_name = obs.get('f1_sig_name', '')
    f2_name = obs.get('f2_sig_name', '')
    fid1 = _SIG_TO_ANTEX.get(f1_name)
    fid2 = _SIG_TO_ANTEX.get(f2_name)
    ...
    for fid in (fid1, fid2):
        sat_pat = antex_parser.get_sat_pattern_fallback(sv, fid, epoch_t)
        rcv_pat = antex_parser.get_recv_pattern(receiver_ant_type, fid)
        ...
        # Receiver side: PCO in N-E-U, LOS in our ecef_to_enu rows
        # are [E, N, U] — align accordingly.
    ...
    # α₁ = f1² / (f1² − f2²), α₂ = −f2² / (f1² − f2²)
    f1_hz = _C_LIGHT / obs.get('wl_f1', 1.0)
    f2_hz = _C_LIGHT / obs.get('wl_f2', 1.0)
