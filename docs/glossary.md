...
| **rank-deficient** | Property of a linear system where some direction in state space produces no observable output.  In a Kalman filter: the observability matrix `H^T R^-1 H` doesn't have full rank.  Rank-deficient systems cannot uniquely estimate every state from observations alone — they rely on process noise and priors to stay bounded.  "Near rank-deficient" is more common in practice: the smallest eigenvalue of `H^T R^-1 H` is very small but nonzero, so drift along the corresponding direction is bounded but slow. |
| **null vector** | A direction in state space where `H · Δx = 0`: the filter can move along this direction without any observation residual changing.  Exact null vectors exist only in strictly rank-deficient systems; in practice we see *near*-null vectors with `H · Δx` small but nonzero.  Found by eigenvalue decomposition of the covariance matrix `P` (smallest-eigenvalue eigenvector) or the observability matrix. |
| **null mode** | A specific near-null vector in the PPP filter that couples three states: receiver clock, ZTD × mapping function, and common-ambiguity drift — `(δclk, δZTD·m_wet, δN_common)`.  The three can shift together and leave residuals ~unchanged.  Excited by static-receiver + smooth precise clocks + single or marginal-geometry constellations.  Not eliminated by any filter tuning — it's geometric — but stays quiescent when the observation model is complete enough that residuals have no systematic component to drive drift along the null direction.  PRIDE on ABMF 2020/001 GPS+GAL reaches 3.3 mm 3D because its complete obs-model keeps the null mode silent; our harness on the same data reaches 5.7 m because cm-scale residuals from obs-model gaps × EKF gain along the null direction produce ~100× amplification over 24 h.  See `project_to_main_pride_ablation_20260423.md` and `feedback_math_check_and_set_ceiling.md`. |
