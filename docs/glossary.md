...
| **PFR** | Post-Fix Residual monitor.  Watches per-SV post-fit PR residuals after NL integers are fixed, to catch wrong integer solutions that the LAMBDA ratio and bootstrap success-rate gates missed.  A wrong integer shifts position to stay self-consistent with its own fix, so σ looks fine — but PR residuals grow as geometry changes.  Today's implementation uses three escalation levels L1/L2/L3; planned redesign splits this into Job A (wrong-integer detection on NL_PROVISIONAL SVs) and Job B (elev-weighted retirement of setting SVs in NL_VALIDATED) with a rare host-level alarm — see `docs/sv-lifecycle-and-pfr-split.md`. |
| **SvAmbState** | Per-satellite ambiguity state machine.  States: FLOAT (no AR), WL_FIXED (MW wide-lane integer only), NL_PROVISIONAL (LAMBDA fixed, on probation), NL_VALIDATED (survived geometry-change test), RETIRING (descending below mask), BLACKLISTED (slip or repeat failure).  Orthogonal to the host-level AntPosEstState.  Host RESOLVED = ≥ N SVs in NL_VALIDATED.  See `docs/sv-lifecycle-and-pfr-split.md`. |
