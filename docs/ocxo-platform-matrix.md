...
## On E810, the OCXO is not our PPS DO (but it is our PTP DO)

The E810 has two independent output domains, and we control only one:

| Domain | Our DO? | Why |
|---|---|---|
| **PTP packet timestamps** | **Yes** | Stamped by GLTSYN counter (= PHC), steered by `adjfine`, full 812.5 MHz (~1.23 ns) resolution |
| **1PPS output (SMA/SDP)** | **No** | Generated by ZL30795 DPLL, controlled by Intel firmware, `adjfine` doesn't touch it |
| **SyncE recovered clock** | **No** | Also DPLL-driven |

For PTP, the E810 is an excellent DO: the OCXO's short-term stability
passes through to the GLTSYN counter as physically stable ticks, our
servo handles long-term GNSS tracking via `adjfine` (digital rate
overlay on INCVAL), and PTP clients downstream see time that *we*
disciplined.  This is exactly what ts2phc + ptp4l deliver, and it's
why "happy" E810 timing users (ScottStuff.net, Red Hat T-GM deployments,
etc.) report single-digit-ns PTP accuracy — they live entirely in the
PTP packet domain and never need the 1PPS to reflect their servo's work.

For 1PPS output, the OCXO is not our DO.  The 1PPS on SMA/SDP comes
from the ZL30795 DPLL, which Intel firmware controls.  Our `adjfine`
writes have no effect on that signal.  Any use case that cares about a
physical PPS edge — TICC characterization, driving downstream equipment,
validating the servo against an independent measurement chain — cannot
use the E810's 1PPS as its output.

The natural parallel to the Timebeat OTC's Renesas 8A34002 ClockMatrix
breaks down only on the PPS/analog side.  On the OTC we read the chip's
TDC for measurement *and* write a frequency control word (FCW) to steer
the OCXO directly, running our PI servo entirely in userspace.  On E810
we can steer the PHC (for PTP) but not the OCXO (for PPS).
...
The E810 serves two roles, and the "not our DO" framing applies to
only one:

**As a PTP grandmaster**: the E810 *is* our DO.  The GLTSYN counter
(PHC) is clocked by the OCXO, stamped at 812.5 MHz resolution, and
steered by our servo via `adjfine`.  PTP clients receive time that we
disciplined, with the OCXO's short-term stability passing through as
physically stable ticks.  Standard Linux timing tools (ts2phc, ptp4l,
phc2sys) work well here.  This is the path that makes other users
happy — and it should make us happy too for the PTP use case.

**As a PPS source for analog/TICC measurement**: the OCXO is **not**
our DO.  Intel's firmware owns the ZL30795 DPLL and the 1PPS output
it generates.  Our `adjfine` writes do not reach the OCXO or the
DPLL.  The PHC's software incrementer is our only knob, and it is
*softer* than even the i226 TCXO because it adds zero physical
inertia — pure arithmetic on top of a firmware-controlled oscillator.
...
shines through at the 1PPS output) requires a timing chip on a
host-accessible bus with an open register map.  That is exactly what
the Timebeat OTC provides and exactly what the E810 does not for PPS.

**Bottom line**: E810 is a strong PTP GM platform but not a moonshot
PPS platform.  Use it for PTP transport; use i226 + TICC or
ClockMatrix for PPS characterization and servo R&D.
