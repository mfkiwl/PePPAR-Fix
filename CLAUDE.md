...
| [sv-lifecycle-and-pfr-split.md](docs/sv-lifecycle-and-pfr-split.md) | **Per-SV state machine** (FLOAT → WL_FIXED → NL_PROVISIONAL → NL_VALIDATED → RETIRING; BLACKLISTED) orthogonal to the host-level `AntPosEstState`.  Splits PFR into Job A (wrong-integer detection on provisional fixes) and Job B (elev-weighted retirement of setting SVs).  Replaces today's PFR L1/L2/L3 cascade with per-SV actions + rare host-level monitor.  Motivated by day0419d ~2 PFR L3/hr observations. |
